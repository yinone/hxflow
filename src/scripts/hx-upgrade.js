 ...
 *   3. 更新当前项目 CLAUDE.md / AGENTS.md 中的 harness 标记块（如在项目中运行）
 *
 * 三层架构升级原则：
...
if (options.help) {
  ...
  更新框架到最新版：
    ...
    3. 更新当前项目 CLAUDE.md / AGENTS.md 中的 harness 标记块
...
const PACKAGE_ROOT = resolve(FRAMEWORK_ROOT, '..')

console.log(`\n  Harness Workflow · upgrade${dryRun ? ' (dry-run)' : ''}`)
console.log(`  系统层: ${PACKAGE_ROOT}\n`)

// ── Step 1: 更新系统层 ──

console.log('  Step 1: 更新系统层...')
const isGitRepo = existsSync(resolve(PACKAGE_ROOT, '.git'))
try {
  if (!dryRun) {
    if (isGitRepo) {
      const output = execSync('git pull', { cwd: PACKAGE_ROOT, encoding: 'utf8' })
...
try {
  if (!dryRun) {
    const hxBin = resolve(PACKAGE_ROOT, 'bin', 'hx.js')
...
// ── Step 3: 更新 Agent 说明文件标记块（如在项目中运行）──

console.log('  Step 3: 更新 Agent 说明文件标记块...')
const projectRoot = findProjectRoot(process.cwd())
upgradeAgentInstructions(projectRoot, summary)
...
function upgradeAgentInstructions(projectRoot, summary) {
  for (const fileName of ['CLAUDE.md', 'AGENTS.md']) {
    upgradeInstructionFile(projectRoot, fileName, summary)
  }
}

function upgradeInstructionFile(projectRoot, fileName, summary) {
  const targetPath = resolve(projectRoot, fileName)

  if (!existsSync(targetPath)) {
    summary.skipped.push(`${fileName} (不在项目中，跳过)`)
    return
  }
  ...
  if (!content.includes(HARNESS_MARKER_START)) {
    summary.warnings.push(`${fileName} 中未找到 harness 标记块，跳过（可运行 hx-init 重新安装）`)
    return
  }
  ...
  if (existsSync(configPath)) {
    try {
      ...
      summary.warnings.push(`.hx/config.yaml 解析失败，使用默认 ${fileName} 标记块: ${err.message}`)
    }
  }

  const newBlock = buildHarnessBlock({ requirementDoc, planDoc })
  const currentBlock = content.match(
    new RegExp(`${escapeRegExp(HARNESS_MARKER_START)}[\\s\\S]*?${escapeRegExp(HARNESS_MARKER_END)}`)
  )?.[0]

  if (currentBlock === newBlock) {
    summary.skipped.push(`${fileName} (标记块无变化)`)
    return
  }
  ...
  summary.updated.push(`${fileName} (harness 标记块)`)
}
