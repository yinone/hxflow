...
describe('command contracts', () => {
  ...
  it('keeps shared contracts split by concern', () => {
    ...
    expect(commandsReadme).toContain('## 提示词与脚本边界')
    expect(commandsReadme).toContain('确定性规则')
    expect(commandsReadme).toContain('优先放在脚本或专用 contract')
  ...
  it('keeps the main-chain parameter model aligned', () => {
    ...
    expect(hxRun).toContain('lastRun` 与 task 状态一致')
    expect(hxRun).toContain('输入完整性校验是固定步骤')
    expect(hxRun).toContain('校验输入是否完整')
    expect(hxRun).toContain('progress contract 与脚本实现回写')
